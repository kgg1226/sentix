...
> 파괴 방지 + 결정론적 검증 + 입력 강화 + 자동 학습 — 네 겹의 품질 레이어로 AI 코딩의 하한과 상한을 모두 관리합니다.
...
```
문제 1 — 하한이 불안정하다 (가끔 큰 사고를 친다):
  - 기존 기능을 실수로 삭제한다
  - 테스트를 깨뜨리고 모른 척 넘어간다
  - export를 바꿔서 다른 모듈을 망가뜨린다
  - 같은 실수를 세션마다 반복한다

문제 2 — 상한이 평범하다 (입력이 빈약하면 결과도 빈약하다):
  - "로그인 만들어" 같은 모호한 요청 → 모호한 결과
  - 프로젝트 고유 규칙을 매번 설명해야 한다
  - AI가 자기 코드를 자기가 리뷰해도 사각지대가 같다
```

Sentix는 **네 겹의 레이어**로 두 문제를 모두 다룹니다:

```
Layer 1 — 파괴 방지 (하한 고정):
  ✅ 기존 기능 삭제 차단                (하드 룰 6개)
  ✅ 허용 범위 밖 파일 수정 차단          (SCOPE 검증)
  ✅ Write/Edit 훅으로 물리적 강제        (PreToolUse)

Layer 2 — 결정론적 검증 (AI가 아닌 기계가 검증):
  ✅ eval/innerHTML/시크릿 패턴 감지      (Quality Gate)
  ✅ 구문 오류, 테스트 회귀, 보안 취약점    (5종 자동 검사)
  ✅ 기계 검증 통과 후에만 리뷰 진행       (Gate 강제)

Layer 3 — 입력 강화 (상한을 올리는 시작점):
  ✅ 프로젝트 제약 자동 주입              (constraints.md)
  ✅ 빈약한 요청에 구조화 질문 생성        (Spec Questions)
  ✅ 과거 실패 패턴을 프롬프트에 반영       (Spec Enricher)

Layer 4 — 자동 학습 (시간이 지날수록 개선):
  ✅ 검증 실패 → 제약에 자동 추가         (Feedback Loop)
  ✅ 반복 패턴 → 규칙으로 자동 승격        (Lesson Promoter)
  ✅ 프로젝트가 쓸수록 constraints 성장     (누적 지식)
```

**고급 기능 (opt-in):**

```
Layer 5 — 다중 생성 (--multi-gen):
  ✅ dev를 3번 독립 실행 (단순/견고/우아 접근법)
  ✅ Quality Gate로 각각 점수 매기고 최선 자동 선택
  ✅ 사용: sentix run "요청" --multi-gen

Layer 6 — 이종 모델 리뷰 + 적대적 프롬프트 (--cross-review):
  ✅ pr-review를 OpenAI/Ollama 등 다른 모델로 실행 (진짜 독립 검증)
  ✅ 리뷰어에게 "최소 2개 문제 찾기" 의무 부여 (게으른 APPROVED 방지)
  ✅ 사용: sentix run "요청" --cross-review openai
```

**한 줄 요약: Sentix는 AI 코딩의 하한을 고정하고, 상한을 끌어올리고, 시간이 지나면서 스스로 개선되는 파이프라인입니다.**

---

## 어떻게 작동하나요?

파이프라인으로 작업을 단계별로 나누고, 각 단계 사이에 **기계적 검증**을 끼워넣습니다.

```
당신: "이거 해줘"
  │
  ▼
[Spec Questions]  — 요청이 빈약하면 자동으로 보강 질문 생성
  │
  ▼
[Spec Enricher]   — 프로젝트 제약 + 과거 실패 패턴 자동 주입
  │
  ▼
planner → dev → [Quality Gate] → pr-review → finalize
                      │
                      │ 기계적 검증 5종
                      │ (eval 감지, 구문 오류, 테스트 회귀, 보안 취약점)
                      │
                      ▼
               [Feedback Loop] — 실패 패턴을 자동으로 기록
                                → 다음 실행에서 같은 실수 방지
```

핵심은 **에이전트를 여러 개 쓰는 것**이 아니라 (같은 AI가 역할만 바꿉니다), **단계 사이에 기계적 검증을 끼우는 것**입니다.
