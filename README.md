...
**AI 코딩의 안전벨트 — 기존 기능 삭제 방지, 결정론적 코드 검증, 실패 패턴 자동 학습.**

> AI가 코드를 더 잘 짜게 만들진 못합니다. 대신 AI가 가끔 저지르는 파괴적 실수를 막습니다.
...
AI가 한 번에 완벽한 코드를 짜는 것은 Sentix도 보장하지 못합니다. **같은 모델이 돌아가기 때문입니다.**

Sentix가 하는 것은 **그 "가끔"을 막는 것**입니다:

```
Sentix가 하는 것:
  ✅ 기존 기능을 실수로 삭제 못 하게 막는다     (하드 룰 6)
  ✅ 테스트를 깨뜨리면 코드가 자동으로 잡는다   (Quality Gate)
  ✅ 허용된 파일만 수정할 수 있게 강제한다      (SCOPE 검증)
  ✅ 보안 위험 패턴을 기계적으로 검출한다       (Banned Patterns)
  ✅ 과거 실패를 기억해서 반복을 막는다        (Feedback Loop)
...
**한 줄 요약: Sentix는 AI 코딩의 안전벨트입니다. 차가 더 빨리 가게 하진 않지만, 사고 날 때 값어치를 합니다.**
