...
| 레이어 | 역할 | `sentix run` | 대화 모드 |
|---|---|:---:|:---:|
| **L1** 파괴 방지 | 하드 룰 6개 + PreToolUse 훅 | ✅ | ✅ 훅 작동 |
| **L2** 결정론적 검증 | eval/구문오류/테스트회귀/보안 | ✅ | ❌ |
| **L3** 입력 강화 | constraints.md + 구조화 질문 | ✅ | ❌ |
| **L4** 자동 학습 | 실패 → 제약 자동 추가 | ✅ | ❌ |
| **L5** 다중 생성 | dev × 3 → 최선 선택 (`--multi-gen`) | ✅ | ❌ |
| **L6** 이종 검증 | 외부 AI 리뷰 (`--cross-review`) | ✅ | ❌ |

> **`sentix run`** = L1~L6 전체 작동 (파이프라인 실행 코드가 패키지 내부에서 구동)
>
> **대화 모드** (Claude Code에서 직접 대화) = L1만 작동 (훅 + CLAUDE.md 지시문)
> L2~L6은 `sentix run`이 패키지 내부 코드(`node_modules/sentix/src/`)를 실행해야 작동합니다.
> 대화 모드에서 Claude는 프로젝트 루트만 보므로 이 코드에 접근하지 않습니다.
