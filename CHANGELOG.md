...
- agent prompts gain agentic_loop, 3P, discernment + fact-grounded coding rule
- sentix init deploys system prompt template; doctor checks it
- add agent routine pattern to state-changing commands
...
- `feat-011`: Claude Academy 17강 요약(Core Principles Digest) 과 Sentix System Prompt v1.0 템플릿...
- `feat-012`: CLAUDE.md + FRAMEWORK.md + .sentix/rules/ 에 참조 문서 기반 3개 섹션 추가: (1) 4D 협업 프레임워...
...
- replace 2-axis config.toml pattern with 3-axis (target + section + intent)
...
- clarify Visual Perception as planned, not implemented
- add 4D / uncertainty / anti-patterns to CLAUDE & FRAMEWORK
- rebuild availability matrix to 5 columns, code-verified
- port Claude Academy core principles + system prompt template
...
- close bug-013
- close feat-009..014 + finalize sprint handoff + README sync
- plan Claude Academy alignment sprint (feat-010 ~ feat-014)
